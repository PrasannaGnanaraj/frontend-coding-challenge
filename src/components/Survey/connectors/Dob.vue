      ...
      this.$validator.validate().then(result => {
        if (result && !this.feedback) {
          this.sendToAPI().then(
            ...
            error => {
              alert(
                `Survey Submission Failed due to ${error.response.data.error} . please try again after fixing the error`
              )
            }
          )
        }
